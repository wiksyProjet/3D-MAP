$(document).ready(function(){
	...
	$("#showMdpUser").click(function(){
		User.decryptMdp($('#showMdpUser').val());
	})
	...
	$(".editionFacture").click(function(){
		if(InterfaceFacture.modif){
			InterfaceFacture.editFacture($(this).attr("data-type"));
		}
		else{
	...
	$(document).on("click", ".imgResult", function(){
		...
		'<li><img class="indexImg" data-id="'+$(this).attr("data-id")+'" src='+publicSite+'?media=image&id='+$(this).attr("data-id")+'&width=100></li>'
		);
		$(".indexImg").draggable({
			revert : "invalid",
		...
		$(document).on('dragenter', '.drop', function(event) {
