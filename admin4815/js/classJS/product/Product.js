var Product = {
	...
	correctProduct : function(){
		...
		$("#productForm").find("input[type=text]").each(function(index, elem){
			if(!$(elem).val() && $(elem).attr("id")!="uriModel3D" &&  $(elem).attr("id")!="uriVideo" && $(elem).attr("id")!="libelleStock" && $(elem).attr("id")!="productType")
				b = false;
		}.bind(this));
